...
if ($branch -eq $snapshot.localFeatureBranch -or $branch -eq $snapshot.chapter2Branch) {
    ...
    $sourceChecks = @(
        ...
        @{ Path = 'source\NativeVR\src\vr_hands.hpp'; Expected = $candidate.source.vrHandsHeaderSha256 },
        @{ Path = 'source\NativeVR\src\vr_tools.cpp'; Expected = $candidate.source.vrToolsSha256 },
        @{ Path = 'source\NativeVR\src\vr_tools.hpp'; Expected = $candidate.source.vrToolsHeaderSha256 },
