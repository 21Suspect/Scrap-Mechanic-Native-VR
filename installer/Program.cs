...
[assembly: AssemblyTitle("Scrap Mechanic VR Patcher")]
[assembly: AssemblyDescription("One-file installer, launcher, verifier, and restorer for Scrap Mechanic Native VR")]
[assembly: AssemblyCompany("Scrap Mechanic VR Community Project")]
[assembly: AssemblyProduct("Scrap Mechanic VR")]
...
namespace ScrapMechanicVRPatcher
{
    ...
    {
        internal const string Version = "1.17.0-20260723";
        internal const string GameBuild = "22163681";
        internal const string GameExeHash = "9D8B5F97171413DB1C284FC36B983173CB08E272AF3DF427F7DCDA360A3FD1FE";
        internal const string AddonHash = "EE3511DBBA61E38F052CA777A22944DDEE5275D8B0BAF3B6D4F176FF3EE1215F";
        internal const string PlayerHash = "C8256E3B38C00EDDC796FAA8DA65F5B5A98203E5BDB5FAA9DF419686ED4762E5";
        internal const int ManagedFileCount = 15;
    ...
    internal static class PackageManager
    {
        ...
        private static bool PackageIsValid()
        {
            ...
            string addon = Path.Combine(PackageDirectory, "payload", "Release", "scrap_native_vr.addon64");
        ...
        internal static void EnsureExtracted()
        {
            ...
            try
            {
                ...
                string stagedAddon = Path.Combine(temporaryDirectory, "payload", "Release", "scrap_native_vr.addon64");
                if (!File.Exists(Path.Combine(temporaryDirectory, "Patcher.ps1")) ||
                    !File.Exists(Path.Combine(temporaryDirectory, "manifest.json")) ||
    ...
    internal static class GameLocator
    {
        ...
        internal static bool LooksInstalled(string root)
        {
            try
            {
                string addon = Path.Combine(root, "Release", "scrap_native_vr.addon64");
                string player = Path.Combine(root, "Survival", "Scripts", "game", "SurvivalPlayer.lua");
                return File.Exists(addon) && File.Exists(player) &&
                       String.Equals(Hashing.Sha256(addon), BuildInfo.AddonHash, StringComparison.OrdinalIgnoreCase) &&
                       String.Equals(Hashing.Sha256(player), BuildInfo.PlayerHash, StringComparison.OrdinalIgnoreCase);
            }
            catch { return false; }
        }
    ...
    internal static class Shortcuts
    {
        internal static readonly string ManagerExecutable = Path.Combine(
            PackageManager.StateRoot, "ScrapMechanicVR.exe");

        private static string DesktopShortcut
        {
            get { return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory), "Start Scrap Mechanic VR.lnk"); }
        }

        private static string StartMenuDirectory
        {
            get { return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Programs), "Scrap Mechanic VR"); }
        }
        ...
        internal static void Install(string gameRoot)
        {
            ...
            CreateShortcut(DesktopShortcut, ManagerExecutable, "--start", "Start Scrap Mechanic in native OpenXR VR", gameIcon);
            CreateShortcut(Path.Combine(StartMenuDirectory, "Start Scrap Mechanic VR.lnk"), ManagerExecutable, "--start", "Start Scrap Mechanic in native OpenXR VR", gameIcon);
            CreateShortcut(Path.Combine(StartMenuDirectory, "Manage or Uninstall Scrap Mechanic VR.lnk"), ManagerExecutable, "", "Manage, verify, or uninstall Scrap Mechanic VR", gameIcon);
        }
    ...
    internal sealed class MainForm : Form
    {
        ...
        internal MainForm()
        {
            Text = "Scrap Mechanic VR - Installer and Launcher";
            ...
            title.Text = "Scrap Mechanic Native VR";
            ...
            subtitle.Text = "NATIVE OPENXR INTEGRATION  /  META QUEST";
        ...
        private void RefreshState()
        {
            ...
            modStatus.Text = installed
                ? "●  VR MOD   Current version installed — " + BuildInfo.ManagedFileCount + " managed files"
                : managedInstall
                    ? "●  VR MOD   Installation is incomplete or modified — Restore & Reinstall is available"
                    : "●  VR MOD   Not installed";
            modStatus.ForeColor = installed ? Success : managedInstall ? Warning : MutedText;

            installButton.Text = managedInstall && !installed ? "RESTORE & REINSTALL" : "INSTALL VR MOD";
            installButton.Enabled = compatible && !installed;
            ...
            uninstallButton.Enabled = installed || managedInstall;
        ...
        private void InstallClicked(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(this,
                ...
                "Install Scrap Mechanic VR", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) != DialogResult.OK)
                return;
            ...
            try
            {
                ...
                MessageBox.Show(this,
                    "Scrap Mechanic VR is installed.\n\nStart Meta Quest Link, connect the headset, then use the new 'Start Scrap Mechanic VR' shortcut.",
        ...
        private void StartClicked(object sender, EventArgs e)
        {
            try
            {
                string root = gamePath.Text.Trim();
                GameLocator.Save(root);
                Append("Starting the active OpenXR runtime and Scrap Mechanic VR...");
    ...
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (args.Length > 0 && String.Equals(args[0], "--start", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!GameLocator.IsGameRoot(root) || !GameLocator.LooksInstalled(root))
                        throw new InvalidOperationException("The managed Scrap Mechanic VR installation could not be found. Run the VR manager and reinstall or verify it.");
                    PatcherRunner.StartDetached(root);
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show(ex.Message, "Scrap Mechanic VR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
