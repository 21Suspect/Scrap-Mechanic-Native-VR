...
param(
    ...
    [string]$GamePath,
    [string]$StateRoot = (Join-Path $env:LOCALAPPDATA 'ScrapMechanicVR'),
    [switch]$Force
)
...
function Get-Manifest {
    ...
    if ($manifest.formatVersion -ne 1 -or -not $manifest.files) {
...
function Install-Patch([string]$Root, $Manifest) {
    ...
    foreach ($entry in $Manifest.files) {
        ...
        if ($status.Status -eq 'patched') { continue }
        ...
        $plan += [pscustomobject]@{ Entry = $entry; Status = $status }
    }
    ...
    try {
        foreach ($item in $plan) {
            ...
            $record = [ordered]@{
                ...
                existed = [bool](Test-Path -LiteralPath $target -PathType Leaf)
                originalSha256 = $item.Status.Hash
...
switch ($Action) {
    ...
    'Start' {
        $launcher = Join-Path $root 'NativeVR\Start-NativeVR.ps1'
        if (-not (Test-Path -LiteralPath $launcher)) {
            throw 'The VR launcher is not installed. Run the Install action first.'
        }
        & $launcher
    }
}
