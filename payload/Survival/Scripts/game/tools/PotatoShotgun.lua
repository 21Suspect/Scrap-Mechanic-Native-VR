...
function PotatoShotgun.client_onUpdate( self, dt )
	...
	if Chapter2VR and Chapter2VR.seatedGunUpdate then
		Chapter2VR.seatedGunUpdate( self, "f6250bf4-9726-406f-a29a-945c06e460e5", dt )
	end
	...
	if self.tool:isLocal() then
		...
		local finalDispersion = clamp( self.movementDispersion + spreadFactor * recoilDispersion, 0.0, 1.0 );
		self.tool:setDispersionFraction( 0, finalDispersion )
