...
local SplashDamage = 35
local SplashDamageRadius = 1.25

PotatoLauncher = class()

function PotatoLauncherSplash( world, hitPos )
	sm.fire.igniteSphere( hitPos, 1.0, true )
	local contacts = sm.physics.getSphereContacts( hitPos, SplashDamageRadius, world, sm.physics.filter.character )
	for _,character in ipairs( contacts.characters ) do
		if sm.exists( character ) then
			...
			if unit and sm.exists( unit ) then
				...
				local direction = ( character:getWorldPosition() - hitPos ):safeNormalize( sm.vec3.new( 1, 0, 0 ) )
				sm.event.sendToUnit( unit, "sv_e_receiveDamage", { damage = SplashDamage, impact = direction, hitPos = hitPos - direction * radius, source = "fire" } )
			end
		end
	end
	CablebotManager.Sv_Explosion( hitPos, SplashDamageRadius )
end

local renderables = {
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Base/char_spudgun_base_basic.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Barrel/Barrel_launcher/char_spudgun_barrel_launcher.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Sight/Sight_ironclamp/char_spudgun_sight_ironclamp.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Stock/Stock_stock02/char_spudgun_stock_stock02.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Tank/Tank_gascan/char_spudgun_tank_gascan.rend"
}
...
function PotatoLauncher.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "SpudgunFireball - FireballMuzzel" )
	self.shootEffectFP = sm.effect.createEffect( "SpudgunFireball - FPFireballMuzzel" )
end

function PotatoLauncher.client_onRefresh( self )
	self:loadAnimations()
end

function PotatoLauncher.loadAnimations( self )
...
function PotatoLauncher.client_onEquip( self, animate )
...
function PotatoLauncher.client_onUnequip( self, animate )
...
function PotatoLauncher.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function PotatoLauncher.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function PotatoLauncher.onAim( self, aiming )
	self.aiming = aiming
...
function PotatoLauncher.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function PotatoLauncher.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function PotatoLauncher.onShoot( self, dir )
...
function PotatoLauncher.calculateFirePosition( self )
...
function PotatoLauncher.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function PotatoLauncher.calculateFpMuzzlePos( self )
...
function PotatoLauncher.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), ITEMS.obj_consumable_fireammo, 1 ) then
			...
			if owner then
				sm.projectile.projectileAttack( projectile_flame, Damage, firePos, dir * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf )
			end
...
function PotatoLauncher.cl_onSecondaryUse( self, state )
...
function PotatoLauncher.client_onEquippedUpdate( self, primaryState, secondaryState )
