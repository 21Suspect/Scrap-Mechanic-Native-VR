...
local Damage = 20
local FireVelocity = 50

local WonkShotChance = 20 -- Percentage chance of a wonk shot
local SpreadAngleWonk = 30
local WonkDamage = 10
local FireVelocityWonk = 25

ScrapPotatoRifle = class()

local renderables = {
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Base/char_spudgun_base_basic.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Barrel/Barrel_scrap/char_spudgun_barrel_scrap.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Sight/Sight_can/char_spudgun_sight_can.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Stock/Stock_mop/char_spudgun_stock_mop.rend",
	"$GAME_DATA/Character/Char_Tools/Char_spudgun/Tank/Tank_glove/char_spudgun_tank_glove.rend"
}
...
function ScrapPotatoRifle.client_onCreate( self )
	self.shootEffect = sm.effect.createEffect( "SpudgunScrap - BasicMuzzel" )
	self.shootEffectFP = sm.effect.createEffect( "SpudgunScrap - FPScrapMuzzel" )
	if self.tool:isLocal() then
		self.onGroundCounter = 0
	end
end

function ScrapPotatoRifle.client_onRefresh( self )
	self:loadAnimations()
end

function ScrapPotatoRifle.loadAnimations( self )
	...
	local movementAnimations = {
		...
		sprintLeft = "spudgun_sprint_left",
		sprintRight = "spudgun_sprint_right",
		...
		landLeft = "spudgun_jump_land_left",
		landRight = "spudgun_jump_land_right",
	...
	self.normalFireMode = {
		...
		minDispersionStanding = 0.07,
		minDispersionCrouching = 0.05,
...
function ScrapPotatoRifle.client_onUpdate( self, dt )
	...
	if self.tool:isLocal() then
		...
		self.shootEffectFP:setRotation( rotFP )
	end
	local pos = self.tool:getTpBonePos( "pejnt_barrel" )
	local dir = self.tool:getTpBoneDir( "pejnt_barrel" )
	local effectPosTP = pos + dir * 0.2
	local rotTP = sm.vec3.getRotation( sm.vec3.new( 0, 0, 1 ), dir )
	self.shootEffect:setPosition( effectPosTP )
	self.shootEffect:setVelocity( self.tool:getMovementVelocity() )
	self.shootEffect:setRotation( rotTP )
	...
	if self.tool:isLocal() then
		...
		if isOnGround then
			...
			self.onGroundCounter = self.onGroundCounter + 1
		end

		if not isOnGround and self.onGroundCounter > 2 then
		...
		local spreadFactor = fireMode.spreadCooldown > 0.0 and clamp( (self.spreadCooldownTimer / fireMode.spreadCooldown) * fireMode.spreadFraction, 0.0, 1.0 ) or 0.0
...
function ScrapPotatoRifle.client_onEquip( self, animate )
...
function ScrapPotatoRifle.client_onUnequip( self, animate )
...
function ScrapPotatoRifle.sv_n_onAim( self, aiming )
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function ScrapPotatoRifle.cl_n_onAim( self, aiming )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onAim( aiming )
	end
end

function ScrapPotatoRifle.onAim( self, aiming )
	self.aiming = aiming
...
function ScrapPotatoRifle.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function ScrapPotatoRifle.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function ScrapPotatoRifle.onShoot( self, dir )
...
function ScrapPotatoRifle.calculateFirePosition( self )
...
function ScrapPotatoRifle.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function ScrapPotatoRifle.calculateFpMuzzlePos( self )
...
function ScrapPotatoRifle.cl_onPrimaryUse( self, state )
...
function ScrapPotatoRifle.cl_onSecondaryUse( self, state )
...
function ScrapPotatoRifle.client_onEquippedUpdate( self, primaryState, secondaryState )
...
function ScrapPotatoRifle.cl_tryShoot( self )
	-- Only the tool owner is allowed to shoot
	if sm.localPlayer.getPlayer() ~= self.tool:getOwner() then return end
	...
	if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
		...
		local damage = Damage
		local fireVelocity = FireVelocity

		if math.random() * 100 < WonkShotChance then
			spreadDeg = SpreadAngleWonk
			damage = WonkDamage
			fireVelocity = FireVelocityWonk
			if not vrGunAim then
				dir = dir:rotate( math.rad( 9.5 ), sm.camera.getRight() ) -- 25 m sight calibration (vel=25)
			end
		else
			if not vrGunAim then
				dir = dir:rotate( math.rad( 3.0 ), sm.camera.getRight() ) -- 25 m sight calibration (vel=50)
			end
		end
		...
		if owner then
			sm.projectile.projectileAttack( projectile_potato, damage, firePos, dir * fireVelocity, owner, fakePosition, fakePositionSelf )
		end
