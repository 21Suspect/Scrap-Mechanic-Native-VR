...
function PotatoRifle.client_onUpdate( self, dt )
	...
	local effectPos, rot
	local vrMuzzle = self.tool:isLocal() and g_vrGunAimActive == true and
		g_vrGunMuzzlePosition and g_vrGunDirection and g_vrGunDirection:length() > 0.5

	if self.tool:isLocal() then
		local dir
		if vrMuzzle then
			dir = g_vrGunDirection:normalize()
			effectPos = g_vrGunMuzzlePosition
		else
			dir = sm.localPlayer.getDirection()
			local firePos = self.tool:getFpBonePos( "pejnt_barrel" )
			effectPos = firePos + dir * ( self.aiming and 0.45 or 0.2 )
		end
	...
	if vrMuzzle then
		...
		dir = self.tool:getTpBoneDir( "pejnt_barrel" )
...
function PotatoRifle.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), obj_plantables_potato, 1 ) then
			local firstPerson = self.tool:isInFirstPersonView()
			local vrHandShot = g_vrGunAimActive == true and
				g_vrGunMuzzlePosition and g_vrGunDirection and g_vrGunDirection:length() > 0.5

			local dir = sm.localPlayer.getDirection()

			local firePos = vrHandShot and g_vrGunMuzzlePosition or self:calculateFirePosition()
			local fakePosition = vrHandShot and firePos or self:calculateTpMuzzlePos()
			local fakePositionSelf = fakePosition
			if firstPerson and not vrHandShot then
				fakePositionSelf = self:calculateFpMuzzlePos()
			end

			-- Aim assist
			if not firstPerson and not vrHandShot then
...
function PotatoRifle.client_onEquippedUpdate( self, primaryState, secondaryState )
	if g_vrPrimaryActionAvailable == true then
		if g_vrPrimaryActionDown == true then
			primaryState = self.vrPrimaryTriggerDown and sm.tool.interactState.hold or sm.tool.interactState.start
			self.vrPrimaryTriggerDown = true
		elseif self.vrPrimaryTriggerDown then
			self.vrPrimaryTriggerDown = false
