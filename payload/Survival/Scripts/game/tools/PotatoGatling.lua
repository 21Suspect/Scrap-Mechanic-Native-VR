...
function PotatoGatling.loadAnimations( self )
	...
	self.normalFireMode = {
		fireCooldown = 0.1,
		spreadCooldown = 0.18,
		spreadIncrement = 0.18,
		spreadMinAngle = 0.5,
		spreadMaxAngle = 32,
		fireVelocity = 130.0,

		minDispersionStanding = 0.09,
		minDispersionCrouching = 0.04,

		maxMovementDispersion = 0.3,
		jumpDispersionMultiplier = 2,
		circleDispersionMultiplier = 0.5,
...
function PotatoGatling.client_onUpdate( self, dt )
	...
	if self.tool:isLocal() then
		...
		local spreadFactor = fireMode.spreadCooldown > 0.0 and clamp( self.spreadCooldownTimer / fireMode.spreadCooldown, 0.0, 1.0 ) * fireMode.spreadMultiplier or 0.0
		local circleDispersion = self.movementDispersion - fireMode.minDispersionStanding + self.gatlingWeight * fireMode.circleDispersionMultiplier
		self.tool:setDispersionFraction( clamp( self.movementDispersion + spreadFactor * recoilDispersion, 0.0, 1.0 ), clamp( circleDispersion, 0.0, 1.0 ) )
